function scnTabMakerSkills(h, i, f) {
    ...
    this.buildTextInputControl = function ( id ) {
        ...
        var labelElementData = '<label for="scn_tab_data">Percents #' + id + '</label>';
        var inputElementData = '<input type="text" id="scn_tab_data_' + id.toString() + '" class="txt input-text" name="scn_tab_data" />';

        this.textInputControl = jQuery('<tr><th>' + labelElement + '</th><td>' + inputElement + '</td></tr>');
        this.textInputControlData = jQuery('<tr><th>' + labelElementData + '</th><td>' + inputElementData + '</td></tr>');

        this.parentControl.parents('tbody').append(this.textInputControl);
        this.parentControl.parents('tbody').append(this.textInputControlData);
    ...
    this.buildTabButtons = function (a) {
        if (this.buttonsControl) {
			...
			jQuery('label[for="scn_tab_data"]').parents('tr').remove();
