var scnDialogHelper = {
    ...
    loadShortcodeDetails: function () {
        if (scnSelectedShortcodeType) {
            var a = this;
            jQuery.getScript(afl_paths.libPath + "shortcodes/sc/" + scnSelectedShortcodeType + ".js", function () {
                a.initializeDialog()
            })
        }
    },
        ...
        if (typeof scnShortcodeMeta == "undefined") jQuery("#scn-options").append("<p>Error loading details for shortcode: " + scnSelectedShortcodeType + "</p>");
            ...
            for (var c in a) {
                ...
                case "tab-control":
                    ...
                    break;
                case "skills-control":
                    this.createTabControlSkills(a[c], f, c == 0);
                    break;
                }
    ...
    createColumnControl: function (a, b, c) {
        new ScnColumnMaker(b, 4, c ? "scn-focus-here" : null);
    ...
    createTabControlSkills: function (a, b, c) {
        new scnTabMakerSkills(b, 6, c ? "scn-focus-here" : null);
        b.addClass("scn-marker-skills-control")
    },
    ...
    getSkillsKeyValue: function (a) {
        var b = a.find("#scn-skills-text").text();
        if (a = Number(a.find("select option:selected").val())) return {
            key: "data",
            value: {
                content: b,
    ...
    makeShortcode: function () {
        ...
        jQuery("#scn-options-table td").each(function () {
            ...
            if (e = h.hasClass("scn-marker-skills-control") ? b.getSkillsKeyValue(h) : b.getTextKeyValue(h)) a[e.key] = e.value
        });
    ...
    previewAction: function (a) {
        ...
        jQuery("#scn-preview-iframe").attr("src", afl_paths.libPath + "shortcodes/preview-shortcode-external.php?shortcode=" + encodeURIComponent(this.makeShortcode()))
    },
